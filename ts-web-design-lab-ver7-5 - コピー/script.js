...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm && formMessage) {
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            // --- ダミー送信処理 (サーバーがない場合のデモ) ---
            setTimeout(() => {
                formMessage.textContent = '（デモ）お問い合わせありがとうございます。';
...
function initThreeJS(container) {
    ...
    async function init() {
        try {
            ...
            const geometry = new THREE.BoxGeometry(2.2, 2.2, 2.2); console.log("Geometry created.");

            // アイコン画像のパスリスト
            const iconPaths = [
                'icons/名称未設定のデザイン.png',
                'icons/名称未設定のデザイン (1).png',
                'icons/名称未設定のデザイン (2).svg',
                'icons/名称未設定のデザイン (3).svg',
                'icons/名称未設定のデザイン.png', // 重複
                'icons/名称未設定のデザイン (1).png' // 重複
            ];
                ...
                return new Promise((resolve, reject) => {
                    textureLoader.load(path,
                        function (texture) {
                            ...
                            gradient.addColorStop(0, '#1B3A5F'); // --main-color (ダークブルー系)
                            gradient.addColorStop(1, '#051937'); // 濃いブルー
                            ...
                            if (imgAspect > 1) { // 横長
                                drawWidth = size * 0.8; // 80%の幅
                                drawHeight = drawWidth / imgAspect;
                            } else { // 縦長または正方形
                                drawHeight = size * 0.8; // 80%の高さ
                            ...
                            resolve(new THREE.MeshPhysicalMaterial({
                                ...
                                transmission: 0.4, // 透過度をさらに下げる (より濃く)
                                roughness: 1.0, // 粗さを最大にして光を拡散
                                ior: 1.5,      // 屈折率
                                reflectivity: 0.5, // 反射率
                                side: THREE.DoubleSide, // 両面をレンダリング
                                thickness: 1.0 // 厚みを加えて光の散乱を増やす
                            }));
                        },
